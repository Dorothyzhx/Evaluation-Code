...
Models = {'ours'};% You can add other model like: Models = {'LHM','PCF','Ours'};
modelNum = length(Models);

%Datasets setting 
DataPath = './Dataset/';
Datasets = {'MSRA-B','DUTS-TE', 'DUT-OMRON', 'ECSSD', 'HKU-IS', 'PASCAL-S', 'SOD'};% You may also need other datasets, such as Datasets = { };
...
for d = 1:datasetNum
    ...
    for m = 1:modelNum
        ...
        save([ResPath model],'Smeasure', 'mae', 'adpFm', 'adpEm'); %   
        fprintf(fileID, '(Dataset:%s; Model:%s) Smeasure:%.4f; MAE:%.4f; adpEm:%.4f; adpFm:%.4f.\n',dataset,model,Smeasure, mae, adpEm, adpFm);       
    end
    toc;
